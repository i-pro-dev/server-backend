...
async def get_messages(from_user_id:int, to_user_id:int):
    return await DB.fetch('select message from messages where from_user_id = $1 and to_user_id = $2 or from_user_id = $2 and to_user_id = $1 order by id', from_user_id, to_user_id)
